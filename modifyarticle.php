...
<head><title>Modificare articol</title><meta name="viewport" content="width=device-width, initial-scale=1.0">
...
include_once 'phplib/usersactions.php';
	...
	if ($auth)
	{
		...
		if ($userrole==1 || $sir[0]==1)
		{
			...
			$content=implode(PHP_EOL,$content);//prelucrare continut
			echo '
<form>
<div class="input-group"><label for="titlu">Titlu:</label>
<input type="text" name="titlu" id="titlu" value="'.htmlspecialchars($titlu).'" class="form-control"></div><br>
<div class="input-group">
<label for="continut">Continut(suporta HTML):</label>
<textarea name="continut" id="continut" class="form-control" rows="20" cols="80">'.$content.'</textarea></div><br>
<h4 id="ajaxanswer"></h4><br>
<script src="javascripts/ajax.js"></script>
<input type="button" class="btn btn-primary" onclick="updateArticle()" value="Modifica">
</form>		
		';
		}
		else
			echo 'Nu aveti drepturi1';
	}
	else 
