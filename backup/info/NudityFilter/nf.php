...
{                              #R  G  B
    ...
    function ImageFilter()
    {
        ...
        //initializam culorile "nude"
    }
    
    function GetScore($image)
    {
        $x = 0; $y = 0;
        $img = $this->_GetImageResource($image, $x, $y); //$x devine lungimea si $y inaltimea
        if(!$img) return false;
		...
		//xPoints yPoints si zPoints sunt folosite pentru a delimita anumite zone pe imagine in care daca gasim culori "nude" atunci crestem scorul
        ...
        for($i=1; $i<=$x; $i++)
        {
            for($j=1; $j<=$y; $j++)
            {
                ...
                if($color >= $this->colorA && $color <= $this->colorB) //daca culoarea din punctul i si j se afla in intervalul de culori "nude"
                {
