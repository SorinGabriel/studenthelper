    ...
    if ($userrole==1) {
        if (isset($_POST['grupa']) && !empty($_POST['grupa'])) {
            ...
            if (empty($_FILES['poza']['name']) || !isset($_FILES['poza']['name']))
            {
                ...
                if (!updateOrar($grupa, $poza))
                    echo 'Ceva nu a decurs cum trebuie <button class="btn btn-default" onclick="history.go(-1);">Inapoi </button>';
                else
                    echo 'Orarul a fost adaugat <button class="btn btn-default" onclick="history.go(-1);">Inapoi </button>';
            }
            else
            {
            ...
            echo 'Grupa nu a fost selectata <button class="btn btn-default" onclick="history.go(-1);">Inapoi </button>';
    }
    else echo 'Nu aveti drepturi <button class="btn btn-default" onclick="history.go(-1);">Inapoi </button>';
