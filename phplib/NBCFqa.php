...
function itemExist($id,$uid)
{
    ...
    $sql=$conn->prepare("SELECT count(*) FROM qapost qp WHERE qp.grup=? and qp.user<>? and qp.date=(SELECT min(date) FROM qapost WHERE grup=qp.grup)");
...
function getRatings()
{
    ...
    $rez=array();
    $sql=$conn->prepare("SELECT rqa.user_id,qp.grup,rqa.nota FROM ratingqa rqa,qapost qp WHERE rqa.qa_id=qp.id and qp.date = (SELECT min(date) FROM qapost WHERE grup=qp.grup) and rqa.user_id <> (SELECT user FROM qapost WHERE id=qp.id)");
    if (!$sql->execute())
    {
        $sql->close();
        $conn->close();
