...
class nbcfchest
{
    ...
    function getKNeighbors($user)
    {
        ...
        usort($neighbors,"cmp");
        for ($i=0;$i<$this->k;$i++)
			if (userExist($neighbors[$i][0]))
				array_push($finalrezult, $neighbors[$i][0]);
		
		/*
		echo 'vecini:';
		for ($i=0;$i<$this->k;$i++) if (userExist($neighbors[$i][0])) echo $neighbors[$i][0].'<br>';
		echo '<br>';
        */
    ...
    function setPredictions($user)
    {
        ...
        for ($i = 0; $i < $this->dimr; $i++)
            if ($this->r[$user][$i]==0 && itemExist($i,$user)) {
                $avg = $this->averageR($user);
                $sum = 0;
                for ($j = 0; $j < count($kn); $j++)
                    $sum += ($this->r[$kn[$j]][$i] - $this->averageR($kn[$j]))  * $this->w[$user][$kn[$j]];
                $sum2 = 0;
                for ($j = 0; $j < count($kn); $j++)
    ...
    {
        include_once 'chestionare.php';
        include_once 'usersactions.php';
        $k=0;
        usort($this->predictions,"cmp2");
        for ($i=0;$i<count($this->predictions) && $k<$nr;$i++)
            if ($this->predictions[$i][0]==$user && $this->predictions[$i][2]>5 && !checkIfTaken($this->predictions[$i][1],getUsername($user)) && itemExist($this->predictions[$i][1],$user))
            {
                if ($k==0) echo '<h4>Recomandari chestionare:</h4><h4><ul class="nav nav-pills">';
                $k++;
                echo '<li><a href="quiz.php?id='.htmlspecialchars($this->predictions[$i][1]).'">'.htmlspecialchars(chestionarTitle($this->predictions[$i][1])[0]).'('.htmlspecialchars(chestionarTitle($this->predictions[$i][1])[1]).')</a></li>';
            }
        echo '</ul></h4>';
    }
...
function itemExist($id,$uid)
{
    include_once 'connectdb.php';

    $conn=connect();

    $sql=$conn->prepare("SELECT count(*) FROM chestionare WHERE id=? and user_id<>?");
...
function userExist($id)
{
    ...
    $sql=$conn->prepare("SELECT count(*) FROM utilizatori WHERE user_id=?");
    $sql->bind_param("i",$id);
    if (!$sql->execute())
    {
        $sql->close();
...
function getRatings()
{
    ...
    $rez=array();
    $sql=$conn->prepare("SELECT user_id,chestionar_id,nota FROM ratingchestionare WHERE user_id <> (SELECT user_id FROM chestionare WHERE id=chestionar_id)");
    if (!$sql->execute())
    {
        $sql->close();
        $conn->close();
    ...
    $sql->bind_result($uid,$cid,$nota);
    while ($sql->fetch())
        array_push($rez,array($uid,$cid,$nota));
